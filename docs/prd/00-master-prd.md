...
- **Filter TransactionsScreen**: pill filter **membungkus (flex-wrap)** sehingga semua opsi filter tampil dalam satu layar tanpa scroll horizontal ke kanan. Filter: pencarian (notes/category/spent_by); wallet (hanya wallet aktif yang memiliki transaksi, dikelompokkan _My Wallets_ / _Member Wallets_ dengan badge owner); kategori (hanya yang terpakai, dengan badge tipe); **Logged by / pencatat (khusus mode circle, dari nilai `spent_by` transaksi)**; rentang tanggal (preset Today / Last 7 days / This month / Last 30 days + custom range). Hasil dikelompokkan per hari dengan label _Today_/_Yesterday_; pill "Reset filters" muncul saat ada filter aktif.
